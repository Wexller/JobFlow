...
6. Release
   ...
   - Release candidates are created from `main` as annotated tags named
     `v<SemVer>`.
   ...
   - Build and deploy from the commit SHA behind the validated release tag.
   - Release record is tag + commit SHA + CI run + deploy/build evidence.
