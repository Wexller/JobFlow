...
workspace now runs Postgres-first by default. Real-database verification is
implemented through the `db:check` lane (migrations/seed/repository/http).
...
- The runtime depends on a reachable Postgres instance in PG-first environments.
- Verification should continue to run through `db:check` in local and CI lanes.
