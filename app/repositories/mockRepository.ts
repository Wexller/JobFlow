...
export interface JobflowRepository {
  readonly listVacancies: () => Promise<Result<Vacancy[]>>
  readonly listPipelineEvents: () => Promise<Result<PipelineEvent[]>>
  readonly listInterviews: () => Promise<Result<Interview[]>>
  readonly listOffers: () => Promise<Result<Offer[]>>
...
function parseMockData(): Result<JobflowSnapshot> {
  ...
  return err(createAppError('validation', 'Invalid mock repository data', {
    cause: parsed.error,
  }))
}

export function createMockRepository(): JobflowRepository {
  const snapshotResult = parseMockData()
  ...
  async function readSnapshot(): Promise<Result<JobflowSnapshot>> {
    ...
    return ok(clone(snapshotResult.value))
  }
  ...
  return {
    ...
    async listInterviews() {
      ...
      return result.ok ? ok(result.value.interviews) : result
    },
    async listOffers() {
      const result = await readSnapshot()
      return result.ok ? ok(result.value.offers) : result
    },
    async listPipelineEvents() {
      const result = await readSnapshot()
      return result.ok ? ok(result.value.pipelineEvents) : result
    },
    async listVacancies() {
      const result = await readSnapshot()
      return result.ok ? ok(result.value.vacancies) : result
    },
  }
}
